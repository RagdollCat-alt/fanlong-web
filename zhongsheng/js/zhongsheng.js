...
async function openHotDetail(postId) {
  ...
  try {
    ...
    $("#hot-detail-content").innerHTML = `<section class="hot-detail-sheet">
      ...
      <div class="hot-detail-hero"><div><span>实时热度</span><strong>${escapeHtml(post.displayHeat)}</strong><small>读取于 ${formatTime(status.heatSyncedAt)}</small></div><div><span>历史最好</span><strong>${status.bestStableRank ? `第 ${status.bestStableRank} 名` : "—"}</strong><small>${status.isBoom ? "曾达到爆榜" : "持续观察中"}</small></div><div><span>观察状态</span><strong>${status.settledAt ? "已结算" : "进行中"}</strong><small>截至 ${formatDeadline(status.observationEndsAt)}</small></div></div>
      <section class="hot-detail-trend"><header><b>榜单走势</b><span>每30分钟一个榜单批次，数字为名次</span></header>${renderHotTrend(data.history)}</section>
      <section class="hot-detail-impact"><header><b>预计名誉影响</b><span class="nature-badge ${post.nature}">${natureLabel(post.nature)}</span></header>${targets.length ? targets.map((item) => `<div><span>${escapeHtml(item.name)}</span><b>${Number(item.progress) > 0 ? "+" : ""}${item.progress}</b></div>`).join("") : `<p>本条未设置影响对象，不产生名誉结算。</p>`}<small>最终结果以连续两个榜单批次的稳定名次及结算状态为准。</small></section>
...
async function openAction(type, postId) {
  ...
  const fields = {
    ...
    cool: `<div class="action-preview"><b>扣除当前热度的20%</b><span>系统会在提交前再校验排名、次数与虞元余额</span></div><p>费用按当前排名和本人的操作次数计算，确认后不退还。</p>`,
  };
