...
function requestArchiveAccess(event) {
    if (event) event.preventDefault();

    let savedAccess = null;
    try {
        savedAccess = JSON.parse(sessionStorage.getItem("fanlongArchiveAccess") || "null");
    } catch (error) {
        savedAccess = null;
    }

    if (savedAccess && savedAccess.qq && savedAccess.name) {
        location.href = "archives.html";
        return;
    }

    const existing = document.querySelector('.archive-auth-overlay');
    if (existing) existing.remove();

    const overlay = document.createElement('div');
    overlay.className = 'cyber-modal-overlay archive-auth-overlay';
    overlay.innerHTML = `
        <div class="cyber-modal-content archive-auth-card">
            <div class="cyber-modal-title">ARCHIVE ACCESS</div>
            <div class="cyber-modal-body">
                <p class="mb-4">请输入身份识别码</p>
                <input id="archive-auth-qq" class="archive-auth-input" type="text" inputmode="numeric"
                    autocomplete="off" placeholder="用户 QQ 号">
                <p id="archive-auth-message" class="archive-auth-message"></p>
            </div>
            <button id="archive-auth-submit" class="cyber-modal-btn">确认进入</button>
        </div>
    `;
    ...
    const input = overlay.querySelector('#archive-auth-qq');
    const submitBtn = overlay.querySelector('#archive-auth-submit');
    const message = overlay.querySelector('#archive-auth-message');
    ...
    const verify = async () => {
        ...
        try {
            const response = await fetch(`${API_BASE}${encodeURIComponent(qq)}&t=${Date.now()}`);
            if (!response.ok) throw new Error("UserNotFound");

            const data = await response.json();
            if (!data || !data.name) throw new Error("EmptyUserData");

            sessionStorage.setItem("fanlongArchiveAccess", JSON.stringify({
                qq,
                name: data.name,
                verifiedAt: Date.now()
            }));
